model = dict(
    ...
    backbone=dict(
        ...
        style='pytorch'),
    neck=dict(
        type="SegMANDecoderFPN",
        in_channels=[144, 288, 512],  # SegMAN Encoder
        out_channels=144, 
        num_outs=3,
        feat_sizes=[(40, 100), (20, 50), (10, 25)], # feature map sizes
        upsample_t=(40, 100),  # upsample target size
        dsample_t=(10, 25),    # downsample target size
    ),
    bbox_head=dict(
        type="CLRerHead",
        ...
        use_segman_decoder=False, # set this false. applying segman decoder module doesn't need to be repeated twice.
