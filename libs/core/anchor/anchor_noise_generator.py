...
class AnchorNoiseGenerator(nn.Module):
    """
    Anchor (prior) generator.
    This also generates the extra embedding for generating anchor.
    Adapted from:
    ...
    def __init__(
        ...
        Args:
            ...
            cls_dim (int): dimension of cls embedding layer.
        """
        super(AnchorNoiseGenerator, self).__init__()
        self.num_priors = num_priors
        self.num_offsets = num_points
        self.cls_dim = cls_dim
        self.prior_embeddings = nn.Embedding(self.num_priors, 3)
        self.cls_embeddings = nn.Embedding(self.num_priors, cls_dim) # (Nr, 64)
        
        self.embed_llayer = nn.Linear(self.cls_dim + 3, self.cls_dim)       
        self.init_anchors()
    ...
    def generate_cls_features(self):
        cls_feat = torch.cat((self.cls_embeddings.weight, self.prior_embeddings.weight), dim=-1) # (Nr, self.cls_dim + 3)
        cls_feat = self.embed_llayer(cls_feat) # (Nr, self.cls_dim)
        return cls_feat
    
    
    def generate_anchors(self, anchor_params, prior_ys, sample_x_indices, img_w, img_h, add_noise=False):
        """
        ...
        if add_noise:
            ...
            noise = torch.randn_like(anchor_xs)
            # if the first dim inputs=batch_size * self.num_priors, it repeats the tensor to fir the shape
            # noise = noise.repeat(num_anchors//self.num_priors, 1) 
                 
            # generate gaussian noise
            # clip value to minimum=0.5, maximum=0.5
            noised_anchor = anchor_xs + torch.clip(noise, min=-0.25, max=0.25) # noise must be clipped to min=-0.5, max=0.5
            # noised_anchor = torch.clip(noised_anchor, min=0, max=1.0) # anchor value must be normalized within [0, 1.0]
            
        sampled_xs = anchor_xs[..., sample_x_indices]
        noised_anchor = noised_anchor[..., sample_x_indices]
        
        return anchor_xs, sampled_xs, noised_anchor
