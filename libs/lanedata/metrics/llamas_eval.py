"""Vendored from the CLRNet checkout (clrnet/utils/llamas_metric.py).
Changes: relative llamas_utils import; cache key includes anno_dir;
shapely imported lazily (only the unofficial continuous IoU uses it).
The tuple/dict result fix for empty images is kept from that checkout.
"""
...
from . import llamas_utils  # vendored from CLRNet (MIT, see file header)
...
def continuous_cross_iou(xs, ys, width=30, img_shape=LLAMAS_IMG_RES):
    ...
    pair of points"""
    from shapely.geometry import LineString, Polygon  # lazy: only the unofficial path needs shapely
    h, w = img_shape
...
def eval_predictions(pred_dir,
    ...
    # CLRNet cached to fixed paths regardless of anno_dir (stale-cache hazard when
    # switching splits). The cache key now includes the annotation directory.
    import hashlib
    cache_key = hashlib.md5(os.path.abspath(anno_dir).encode()).hexdigest()[:10]
    os.makedirs('cache', exist_ok=True)
    annotations_path = f'cache/llamas_annotations_{cache_key}.pkl'
    label_path = f'cache/llamas_label_paths_{cache_key}.pkl'
    ...
    # tuple and dictionary are mixed in the results. we need to extract only dictionary to calculate 0.5 to 0.95 iou MAP.
    new_results = []
    for r in results:
        if type(r) == dict:
            new_results.append(r)
        elif type(r) == tuple:
            new_results.append(r[3])
        
    
    for thr in iou_thresholds:
        
        tp = sum(m[thr][0] for m in new_results)
        fp = sum(m[thr][1] for m in new_results)
        fn = sum(m[thr][2] for m in new_results)
