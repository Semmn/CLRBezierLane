...
class SegMANDecoder(torch.nn.Module):
    def __init__(self, image_size=(320, 800), in_channels=64, embed_dim=128, feat_proj_dim=192, num_classes=5, 
                 dropout_ratio=0.01, channel_split=False, interpolate_mode='bilinear', use_rpb=False):
        super(SegMANDecoder, self).__init__()
        image_size = to_2tuple(image_size)
        ...
        self.linear_c4 = MLP(self.in_channels, self.feat_proj_dim)
        self.linear_c3 = MLP(self.in_channels, self.feat_proj_dim)
        self.linear_c2 = MLP(self.in_channels, self.feat_proj_dim)
        ...
        self.image_pool = nn.Sequential(
                                ...
                                ConvModule(self.embed_dim, self.embed_dim, 1, norm_cfg=dict(type='SyncBN', requires_grad=True), act_cfg=dict(type='ReLU')))
        ...
        self.cls_seg = torch.nn.Sequential(nn.Conv2d(self.embed_dim, self.num_classes, kernel_size=1),
                                           nn.Upsample(size=self.ori_image_size, mode=self.interpolate_mode, align_corners=False))
    ...
    def forward_mlp_decoder(self, inputs):
        ...
        # interpolate to c2 image size
        _c4 = torch.nn.functional.interpolate(_c4, size=c2.size()[2:],mode=self.interpolate_mode,align_corners=False).contiguous()
        _c3 = torch.nn.functional.interpolate(_c3, size=c2.size()[2:],mode=self.interpolate_mode,align_corners=False).contiguous()
    ...
    def forward_winssm(self, x: torch.Tensor, c2, c3, c4):
        out = [self.short_path(x), 
                  torch.nn.functional.interpolate(self.image_pool(x),
                        size=x.size()[2:],
        ...
        _out = torch.nn.functional.interpolate(_out,
                size=x.size()[2:],
                mode=self.interpolate_mode,
                align_corners=False)
    ...
    def forward(self, inputs):
        ...
        # output = torch.nn.functional.interpolate(output, size=(self.ori_image_size), mode=self.interpolate_mode, align_corners=False)
        return output