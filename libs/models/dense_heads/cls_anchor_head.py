...
class PosEncoder(nn.Module):
    def __init__(self, d_model, seq_len):
        ...
        seq_index = torch.arange(0, seq_len, step=1, dtype=torch.float).unsqueeze(dim=1)
        even = torch.arange(0, d_model, step=2, dtype=torch.float)
        
        pos_embed[:, 0::2] = torch.sin(seq_index / (10000**(even/d_model)))
        pos_embed[:, 1::2] = torch.cos(seq_index / (10000**(even/d_model)))
...
class FeatCLSAttn(nn.Module):
    ...
    def forward(self, q, kv):
        ...
        return attn_o+q # (B, Nr, C)


@MODELS.register_module()
class CLSAnchorHead(BaseDenseHead):
    def __init__(
        self,
        anchor_generator,
        img_w=800,
        ...
        super(CLSAnchorHead, self).__init__()
        ...
        self.pos_encoder = PosEncoder(d_model=self.prior_feat_channels, seq_len=self.num_priors)
    ...
    def forward(self, x, **kwargs):
        ...
        for stage in range(self.refine_layers):
            ...
            cls_embed = self.pos_encoder(cls_embed) # positional embedding
            ...
            cls_embed = self.pos_encoder(cls_embed)
            ...
            # 6. cls and reg heads
            # cls_features = torch.cat((fc_features.clone(), cls_embed.reshape(batch_size * self.num_priors, -1)), dim=-1)
