...
class AnchorFeatureMapAttn(nn.Module):
    """
    Compute attentions between anchor and feature map.
        ...
        patch_sz: patch size for image tokenizer
        num_heads: number of heads
    """
    def __init__(self, anchor_cin, num_points, num_anchors, feat_cin, patch_sz, num_heads):
        super().__init__()
        self.anchor_cin = anchor_cin
        self.anchor_cout = patch_sz[0] * patch_sz[1] * feat_cin
        self.num_points = num_points
        self.num_anchors = num_anchors
        self.feat_cin = feat_cin
        self.patch_sz = patch_sz
        self.num_heads = num_heads
        
        # query
        self.anchor_feat_encoder = nn.Linear(anchor_cin * num_points, self.anchor_cout, bias=True)
        
        self.key = nn.Conv2d(in_channels=feat_cin, out_channels=feat_cin, kernel_size=(3,3), stride=1, padding=1, bias=True)
        self.value = nn.Conv2d(in_channels=feat_cin, out_channels=feat_cin, kernel_size=(3,3), stride=1, padding=1, bias=True)

        if self.anchor_cout % num_heads != 0:
        ...
        self.head_dim = self.anchor_cout // num_heads
        self.scale = self.head_dim ** 0.5
        
        self.out = nn.Linear(self.anchor_cout, anchor_cin, bias=True)
    ...
    def forward(self, anchor, feat_map):
        """
            anchor: anchor feature that has shape of (B * Nr, C, Ns, 1)
        ...
        query = self.anchor_feat_encoder(anchor) # [B * Nr, C * Ns] -> [B * Nr, Cout (=patch_sz[0] * patch_sz[1] * anchor_cin)]
        qtoken = query.reshape(B, Nr, -1) # [B * Nr, Cout] -> [B, Nr, Cout]
        
        key = self.key(feat_map)
        value = self.value(feat_map)
        
        ktoken = self.__patchfy__(key) # (B, N_patch, Cout)
        vtoken = self.__patchfy__(value) # (B, N_patch, Cout)
        
        # query: (B, NH, Nr, head_dim)
        # key: (B, NH, N_patch, head_dim)
        # value: (B, NH, N_patch, head_dim)
        ...
        attn_o = attn_o.permute(0, 2, 1, 3).reshape(B, Nr, -1) # (B, Nr, Cout)
        
        out = self.out(attn_o) # (B, Nr, Cout) -> (B, Nr, Cin)
        return out
        

@MODELS.register_module()
class CLRerGSRCHead(BaseDenseHead):
    """
    Similar to CLRerNet but extra output from gsrc_fpn is considered.
    """
    def __init__(
        self,
        anchor_generator,
        ...
        num_extra_output=1, # extra feature map output from neck
        extra_channels=64 # output channels of extra feature map
    ):
        super(CLRerGSRCHead, self).__init__()
        ...
        self.num_extra_output = num_extra_output # number of extra feature map output from neck (in the case of gsrc_fpn neck)
        self.extra_channels = extra_channels # number of channels for extra feature map
        
        extra_attention = []
        for _ in range(self.num_extra_output):
            extra_attention.append(AnchorFeatureMapAttn(anchor_cin=self.prior_feat_channels, num_points=self.sample_points, num_anchors=self.num_priors,
                                                             feat_cin=self.extra_channels, patch_sz=(1, 1), num_heads=2))
        self.extra_attention = nn.ModuleList(extra_attention)
        ...
        cls_modules = list()
        reg_modules += [
            nn.Linear(self.fc_hidden_dim + self.num_extra_output * self.fc_hidden_dim, self.fc_hidden_dim),
    ...
    def forward(self, x, **kwargs):
        ...
        feature_pyramid = list(x[:len(x) - self.num_extra_output])
        # e.g. [1, 64, 10, 25], [1, 64, 20, 50] [1, 64, 40, 100], [, ..., extra_output from neck]
        ...
        for stage in range(self.refine_layers): # 0-th
            ...
            # 3. Apply Global Structured Row-Column Attention
            extra_features = []
            for extra_sg in range(self.num_extra_output):
                # [B * Np, C, Ns, 1], [B, C_extra, H_extra, W_extra] -> (Batch, num_prior, dim)
                extra_features.append(self.extra_attention[extra_sg](pooled_features, x[self.refine_layers + extra_sg]))
                fc_features = torch.cat((fc_features, extra_features[extra_sg]), dim=-1)
            
            fc_features = fc_features.view(self.num_priors, batch_size, -1).reshape(
                batch_size * self.num_priors, -1
            )  # [B * Np, Ch + num_extra_output * C_extra]
