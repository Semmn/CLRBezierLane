...
class RowColAttnFPN(nn.Module):
    def __init__(self, in_channels, fpn_channels, rc_layers, feat_size=[(40, 100), (20, 50), (10, 25)]):
        ...
        """
        super(RowColAttnFPN, self).__init__()
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            rc_block = RowColAttnLayer(in_channels=fpn_channels, proj_dim=fpn_channels, feat_size=self.feat_size[i], 
                            attn_drop=0.1, ffn_dim=fpn_channels*4, ffn_act=nn.ReLU, ffn_drop=0.1, num_layers=self.rc_layers)
            
            self.lateral_convs.append(l_conv)
            self.fpn_convs.append(fpn_conv)
            self.rc_blocks.append(rc_block)
    ...
    def forward(self, inputs):
        ...
        # apply rc-attentions
        rc_attns = [
            self.rc_blocks[i](laterals[i]) for i in range(used_backbone_levels)
        ]

        # apply output convolutions (for chaining)
        outs = [self.fpn_convs[i](rc_attns[i]) for i in range(used_backbone_levels)]
