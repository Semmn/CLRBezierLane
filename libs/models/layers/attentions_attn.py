...
class SelfAttention(nn.Module):
    def __init__(self, d_model, out_proj_dim, num_heads=8, dropout_p=0.1):
        ...
        self.out_proj = nn.Linear(d_model, out_proj_dim)
        
    def forward(self, x):
        # x shape: (B, L, C) where B is batch size, L is sequence length, C is channel dimension
        B, L, C = x.shape
        qkv = self.qkv_proj(x)
        ...
        attn = attn.transpose(1, 2).contiguous().view(B, L, C) # (B, H, L, D) -> (B, L, H * D)
        attn = self.out_proj(attn) # add residual connection to the self-attention
        return attn

@MODELS.register_module()
class AttnROIGather(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        super(AttnROIGather, self).__init__()
